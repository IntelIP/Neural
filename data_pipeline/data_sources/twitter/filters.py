...
class FilterManager:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.api_base_url = "https://api.twitterapi.io"
    ...
    async def apply_rule(self, rule: FilterRule) -> Dict[str, Any]:
        """
        Apply a filter rule via TwitterAPI.io API
        ...
        async with httpx.AsyncClient() as client:
            ...
            payload = {
                "value": rule.value,
                "tag": rule.tag,
                "polling_interval": rule.polling_interval
            }
            
            try:
                response = await client.post(
                    f"{self.api_base_url}/twitter/tweet/websocket/rules",
                    headers=headers,
                    json=payload
                )
                ...
                if response.status_code == 200:
                    ...
                    rule.id = result.get('data', {}).get('id')
                    self.active_rules[rule.id] = rule
                    logger.info(f"Filter rule created: {rule.tag}")
                    return result
                else:
                    logger.error(f"Failed to create filter: {response.text}")
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                response = await client.delete(
                    f"{self.api_base_url}/twitter/tweet/websocket/rules/{rule_id}",
                    headers=headers
                )
