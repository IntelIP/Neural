...
async def fetch_game_data():
    ...
    try:
        ...
        if not live_games:
            ...
            logger.info("\nGame State:")
            logger.info(
                f"  Score: {game_state.home_team} {game_state.home_score} - {game_state.away_team} {game_state.away_score}"
            )
            logger.info(
                f"  Win Probability: {game_state.home_team} {game_state.home_win_probability:.1f}%"
            )
            logger.info(
                f"  Situation: {game_state.down} & {game_state.distance} at {game_state.yard_line}"
            )
            ...
            if injuries:
                ...
                for injury in injuries:
                    logger.info(
                        f"  {injury['player']} ({injury['team']}): {injury['status']}"
                    )
...
async def stream_live_games():
    ...
    async def handle_game_event(event):
        ...
        if event.impact_score > 0.5:
            ...
            logger.info(
                f"  Game State: {event.game_state.home_team} {event.game_state.home_score} - {event.game_state.away_team} {event.game_state.away_score}"
            )
            logger.info(
                f"  Win Prob: {event.game_state.home_team} {event.game_state.home_win_probability:.1f}%"
            )
    ...
    try:
        ...
        if leaders:
            ...
            for category, players in leaders.items():
                if players:
                    leader = players[0]
                    logger.info(
                        f"  {category}: {leader['player']} - {leader['displayValue']}"
                    )
        ...
        if scoring_plays:
            ...
            for play in scoring_plays[-5:]:  # Last 5 scores
                logger.info(
                    f"  Q{play['quarter']} {play['clock']}: {play['text'][:80]}..."
                )
                logger.info(
                    f"    Score after: {play['home_score']}-{play['away_score']}"
                )
