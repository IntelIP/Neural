...
import asyncio
import logging
from data_pipeline.unified_stream import UnifiedStreamManager, MarketCorrelation
...
async def track_game_with_markets():
    ...
    async def handle_correlation(correlation: MarketCorrelation):
        ...
        logger.info(f"Price Movement: {correlation.price_before:.3f} → {correlation.price_after:.3f}")
    ...
    async def handle_game_event(event):
        ...
        if event.impact_score > 0.6:
            logger.info(f"⚡ High Impact Event: {event.description} (Score: {event.impact_score:.2f})")
    
    async def handle_market_update(update):
        """Handle market updates"""
        if abs(update['change']) > 0.02:  # Log significant changes
            logger.info(f"📊 Market Move: {update['ticker']} {update['change']:+.3f} to {update['price']:.3f}")
    ...
    try:
        ...
        if correlations_found:
            ...
            for corr in correlations_found:
                logger.info(f"• {corr.event.type.value}: {corr.price_change_pct:+.2f}% on {corr.market_ticker}")
...
async def monitor_all_nfl_games():
    ...
    async def handle_correlation(correlation: MarketCorrelation):
        ...
        if abs(correlation.price_change_pct) > 3:  # 3% threshold
            logger.info(f"💥 Significant Impact: {correlation.event.type.value} caused {correlation.price_change_pct:+.2f}% move in {correlation.market_ticker}")
    ...
    try:
        ...
        example_markets = [
            "NFL-GENERIC-HOME-WIN",
            "NFL-GENERIC-TOTAL-OVER"
        ]
        ...
        if touchdown_impacts:
            ...
            logger.info(f"Touchdowns: {len(touchdown_impacts)} events, avg impact {avg_td_impact:.2f}%")
        
        if turnover_impacts:
            avg_to_impact = sum(turnover_impacts) / len(turnover_impacts)
            logger.info(f"Turnovers: {len(turnover_impacts)} events, avg impact {avg_to_impact:.2f}%")
...
async def simulate_correlation_scenarios():
    ...
    from data_pipeline.data_sources.espn.models import GameState, GameEvent
