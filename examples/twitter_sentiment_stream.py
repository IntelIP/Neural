...
async def basic_sentiment_stream():
    ...
    async def handle_sentiment_event(event):
        ...
        logger.info(f"  Period: {event.start_time.strftime('%H:%M:%S')} - {event.end_time.strftime('%H:%M:%S')}")
        logger.info(f"  Tweets: {event.tweet_count} from {event.unique_authors} authors")
    ...
    async def handle_sentiment_shift(shift):
        ...
        logger.warning(f"  From: {shift['from_sentiment']:+.2f} To: {shift['to_sentiment']:+.2f}")
    ...
    async def handle_high_impact(tweet, sentiment):
        ...
        logger.info(f"  Author: @{tweet.author.username} ({tweet.author.followers_count:,} followers)")
        logger.info(f"  Text: {tweet.text[:200]}...")
        logger.info(f"  Sentiment: {sentiment.score:+.2f} ({sentiment.sentiment_label})")
...
async def game_sentiment_tracking():
    ...
    async def handle_sentiment_event(event):
        ...
        if event.market_impact.value in ['high', 'critical']:
            ...
            logger.info(f"📍 Key Moment: Sentiment {event.avg_sentiment_score:+.2f}, "
    ...
    try:
        ...
        if summary:
            ...
            if shifts:
                logger.info("\nMomentum Shifts:")
                for shift in shifts:
                    logger.info(f"  {shift['time'].strftime('%H:%M:%S')}: "
            ...
            if key_moments:
                ...
                for moment in key_moments:
                    logger.info(f"  {moment['time'].strftime('%H:%M:%S')}: "
    ...
    def handle_tweet(tweet):
        ...
        if any(kw in text_lower for kw in injury_keywords):
            ...
            if tweet.author.verified or tweet.author.followers_count > 10000:
                alert = {
                    'player': next((p for p in key_players if p.lower() in text_lower), 'Unknown'),
