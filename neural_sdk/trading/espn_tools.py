...
class ESPNTools:
    ...
    Perfect for correlating game events with Neural market movements.
    """
    ...
    async def get_game_summary(
        self, game_id: str, sport: str = "nfl"
    ) -> Dict[str, Any]:
        """
        Get detailed game summary including play-by-play and win probability.
        This is the MOST VALUABLE endpoint for Neural correlation.
    ...
    async def get_win_probability(
        ...
        Direct correlation to Neural market prices!
        """
        summary = await self.get_game_summary(game_id, sport)

        if "winprobability" in summary:
            latest = (
                summary["winprobability"][-1]
                if isinstance(summary["winprobability"], list)
                else summary["winprobability"]
            )
            return {
        ...
        if "drives" in summary:
            for drive in summary.get("drives", {}).get("previous", []):
                for play in drive.get("plays", []):
                    ...
                    if any(
                        word in play.get("text", "").lower()
                        for word in ["intercepted", "fumble", "touchdown"]
                    ):
                        events.append(
                            {
                                "type": "big_play",
                                ...
                                "impact": (
                                    "HIGH"
                                    if "intercepted" in play.get("text", "").lower()
                                    else "MEDIUM"
                                ),
                                "timestamp": play.get("wallclock"),
                            }
    ...
    async def monitor_games_for_events(
        self, sport: str = "nfl", interval: int = 30
    ) -> None:
        """
        Monitor all active games for market-moving events.
        Perfect for real-time Neural correlation.
        """
        while True:
            scoreboard = (
                await self.get_nfl_scoreboard()
                if sport == "nfl"
                else await self.get_nba_scoreboard()
            )
            ...
            for game in scoreboard.get("events", []):
                ...
                if status == "in":  # Game is live
                    ...
                    # This is where you'd correlate with Neural prices
...
# Example usage for Neural correlation
async def correlate_with_neural():
    """
    Example of how to use ESPN data with Neural markets.
    """
    espn = ESPNTools()
    ...
    for game in scoreboard.get("events", []):
        ...
        # Get win probability (correlates with Neural prices)
