...
class SentimentProbabilityConverter:
    ...
    def combine_probabilities(
        ...
        if confidence_values:
            harmonic_mean = len(confidence_values) / sum(
                1 / c if c > 0 else float("inf") for c in confidence_values
            )
    ...
    def calculate_sentiment_momentum(
        sentiment_history: List[Dict[str, float]], window_minutes: int = 30
    ) -> Dict[str, float]:
        """
        ...
        if len(recent) >= 3:
            ...
            first_half_velocity = (sentiments[mid_point] - sentiments[0]) / max(
                1, mid_point
            )
            second_half_velocity = (sentiments[-1] - sentiments[mid_point]) / max(
                1, len(recent) - mid_point
            )
        ...
        if len(recent) >= 3:
            ...
            trend_strength = (
                abs(increases / (len(sentiments) - 1) - 0.5) * 2
            )  # 0 = random, 1 = consistent
        else:
            trend_strength = 0
    ...
    def sports_specific_adjustments(
        base_probability: float, sport: str, context: Dict[str, any]
    ) -> float:
        """
...
if __name__ == "__main__":
    ...
    sentiment_prob = converter.sentiment_to_probability(
        sentiment_score=0.3, confidence=0.8, base_prob=0.5  # Mildly positive
    )
