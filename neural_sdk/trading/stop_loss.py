...
class DynamicStopLoss:
    ...
    def calculate_stop_loss(
        ...
        Args:
            entry_price: Entry price of position (0-1 for Neural)
        ...
        volatility_multiplier = DynamicStopLoss._calculate_volatility_multiplier(
            volatility
        )

        # Adjust for time decay (tighten stops as event approaches)
        time_multiplier = DynamicStopLoss._calculate_time_multiplier(
            time_to_event_hours
        )
        ...
        min_stop = (
            entry_price * 0.05 if position_type == "YES" else (1 - entry_price) * 0.05
        )
        max_stop = (
            entry_price * 0.20 if position_type == "YES" else (1 - entry_price) * 0.20
        )
        ...
        stop_pct = (
            adjusted_stop_distance / entry_price
            if position_type == "YES"
            else adjusted_stop_distance / (1 - entry_price)
        )
    ...
    def calculate_trailing_stop(
        ...
        if position_type == "YES":
            ...
            )  # highest_price is actually lowest for NO
    ...
    @staticmethod
    def _calculate_sentiment_multiplier(
        sentiment_momentum: float, position_type: str
    ) -> float:
...
class StopLossManager:
    ...
    def get_stop_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "last_update": max(
                (s["updated_at"] for s in self.active_stops.values()), default=None
            ),
        }
...
if __name__ == "__main__":
    ...
    trailing = stop_calc.calculate_trailing_stop(
        entry_price=0.40, current_price=0.55, highest_price=0.60, position_type="YES"
    )
