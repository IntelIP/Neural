...
DEPRECATED: Use neural_web_infra.stream_manager.StreamManager directly
"""
...
class MarketContext:
    ...
    def update_neural(self, data: Dict[str, Any]):
        ...
        if "yes_levels" in data and "no_levels" in data:
            ...
            if yes_volume + no_volume > 0:
                self.orderbook_pressure = (yes_volume - no_volume) / (
...
class DataAggregator:
    ...
    def __init__(
        self,
        neural_api_key: Optional[str] = None,
        ...
        Args:
            neural_api_key: Neural API key (unused - uses env vars)
    ...
    async def initialize(self):
        ...
        # Initialize Neural
        if self.neural_api_key:
            self.neural_client = NeuralWebSocket(
                api_key=self.neural_api_key,
                on_message=self._handle_neural_message,
        ...
        if self.twitter_api_key:
            self.twitter_client = TwitterWebSocket(api_key=self.twitter_api_key)
    ...
    async def start(self):
        ...
        if self.neural_client:
            await self.neural_client.connect()
        ...
        self.tasks.append(asyncio.create_task(self._aggregation_loop()))

        # Start Twitter streaming if available
        if self.twitter_client:
            self.tasks.append(asyncio.create_task(self._stream_twitter()))
    ...
    async def track_market(
        ...
        Args:
            market_ticker: Neural market ticker
        ...
        # Subscribe to Neural channels
        if self.neural_client:
            await self.neural_client.subscribe(
                channel=NeuralChannel.TICKER, market_tickers=[market_ticker]
            )

            await self.neural_client.subscribe(
                channel=NeuralChannel.ORDERBOOK_DELTA, market_tickers=[market_ticker]
            )
    ...
    async def _handle_neural_message(self, msg: Dict[str, Any]):
        """Handle Neural WebSocket messages."""
    ...
    async def _handle_espn_message(self, msg: Dict[str, Any]):
        ...
        for context in self.markets.values():
            if context.game_id == game_id:
                ...
                if msg.get("type") in [
                    ESPNEventType.INJURY.value,
                    ESPNEventType.BIG_PLAY.value,
                ]:
                    await self._notify_opportunity(context)
            ...
            async for tweet in self.twitter_client.stream_tweets():
                ...
                for context in self.markets.values():
                    ...
                    if context.home_team and context.away_team:
                        ...
                        if (
                            context.home_team.lower() in text
                            or context.away_team.lower() in text
                        ):
    ...
    async def stop(self):
        ...
        # Close connections
        if self.neural_client:
            await self.neural_client.close()
    ...
    def get_top_opportunities(self, limit: int = 5) -> List[MarketContext]:
        ...
        sorted_markets = sorted(
            self.markets.values(), key=lambda m: m.opportunity_score or 0, reverse=True
        )
...
async def example_usage():
    ...
    aggregator = DataAggregator(
        neural_api_key=os.getenv("KALSHI_API_KEY"),
    ...
    await aggregator.track_market(
        market_ticker="SUPERBOWL-2025", game_id="401547435", teams=["Chiefs", "Bills"]
    )
