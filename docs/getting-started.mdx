...
pip install "neural-sdk[sentiment]"     # optional research tooling
pip install "neural-sdk[deployment]"    # optional experimental deployment helpers
```
...
The base install covers the supported beta imports, CLI, and paper-trading workflow. Add `trading` only when you need Kalshi execution or streaming; keep `sentiment` and `deployment` for explicitly experimental work.
...
4. Treat Polymarket US as read/stream beta only until live order placement is explicitly promoted.
